...
class PartialRenderer : PartialExpressionComposer<Double, String> {
    ...
    override fun fib(partialRight: String) = "fib$partialRight"
...
open class Parser<E : Any>(private val composer: ExpressionComposer<E>) {
    ...
    private fun ExpressionPrefix<E>.tryExtend(tokenizer: Tokenizer): ExpressionPrefix<E>? = when (this) {
        is ContinuableWithExpression -> {
            ...
            val unaryOperator: UnaryOperator? = tokenizer.tryReadUnaryOperator()
            when {
                number != null -> this.with(composer.number(number))
                unaryOperator != null -> this.withUnaryOperator(unaryOperator)
        ...
        is EndedWithExpression -> {
            ...
            if (operator != null) {
                if (this.prefix is EndedWithUnaryOperator) {
                    val reduced = this.reduced()
                    reduced.extendedWithOperator(operator)
                } else {
    ...
    internal tailrec fun EndedWithExpression<E>.reduced(): EndedWithExpression<E> = when (this.prefix) {
        ...
        is EndedWithUnaryOperator -> this.prefix
            .withOperatorApplied(this.expression)
            .reduced()
    }
    ...
    private fun EndedWithUnaryOperator<E>.withOperatorApplied(rightOperand: E) =
        this.prefix.with(composer.compose(this.operator, rightOperand))
    ...
    private fun ExpressionComposer<E>.compose(
        unaryOperator: UnaryOperator, right: E
    ): E = when (unaryOperator) {
        UnaryOperator.FIB -> fib(right)
    }
...
class PartialParser<E : Any, PE : Any>(
    ...
    private tailrec fun ContinuableWithExpression<E>.toPartialExpressionWith(
        ...
        is EndedWithUnaryOperator -> this.prefix.toPartialExpressionWith(
            ending = partialComposer.compose(this.operator, ending)
        )
    }
...
private data class EndedWithUnaryOperator<out E>(
    val prefix: ContinuableWithExpression<E>,
    val operator: UnaryOperator
) : ContinuableWithExpression<E>()

private fun <E> ContinuableWithExpression<E>.withUnaryOperator(unaryOperator: UnaryOperator) =
    EndedWithUnaryOperator(this, unaryOperator)
...
internal enum class UnaryOperator(val sign: String) {
    FIB("fib")
}
...
internal class Tokenizer(private val expression: String) {
    ...
    fun tryReadUnaryOperator(): UnaryOperator? = UnaryOperator.values().firstOrNull {
        if (expression.length < index + it.sign.length)
            false
        else {
            val sign = expression.subSequence(index, index + it.sign.length)
            return if (sign == it.sign) {
                index += (it.sign.length)
                it
            } else
                null
        }
